...
const explanations = {
  ...
  deer:'A deer is a gentle hoofed animal, and many males grow antlers.',
  ...
  cow:'A cow is a farm animal raised for milk, meat, and other products.',
  ...
  hyena:'A hyena is an African and Asian mammal known for strong jaws and complex social groups.'
};

let lastAnimal = '';

function speakExplanation(name) {
  const key = name.toLowerCase().trim();
  if (!key || key === lastAnimal) return;
  lastAnimal = key;
  const explanation = explanations[key] || `A ${key} is an animal. Listen carefully and repeat the word ${key}.`;
  if (!('speechSynthesis' in window)) return;
  speechSynthesis.cancel();
  const utterance = new SpeechSynthesisUtterance(`The answer is ${key}. ${explanation} Repeat after me: ${key}.`);
...
new MutationObserver(() => {
  ...
  const match = text.match(/This is a\s+(.+?)(?:\.|\s+แปลว่า|$)/i);
  if (match) speakExplanation(match[1]);
