...
const localQuestion = (maxNumber) => {
  ...
  while (choices.size < 4) {
    choices.add(Math.max(0, answer + Math.floor(Math.random() * 25) - 12));
  }
...
async function getQuestion() {
  ...
  try {
    const response = await fetch(`/api/question?min=1&max=${maxNumber}`);
    if (!response.ok) throw new Error("API unavailable");
    return await response.json();
  } catch {
    return localQuestion(maxNumber);
  }
}
...
function drawFingerCursor(x, y, pinching) {
  ...
  handContext.fillStyle = pinching
...
function processHandResults(results) {
  ...
  // นิ้วชี้ = จุดที่ 8, นิ้วโป้ง = จุดที่ 4
  // กล้องถูก mirror จึงกลับค่า X เพื่อให้ตำแหน่งตรงกับภาพ
...
async function enableCamera() {
  ...
  try {
    ...
    messageText.textContent =
      "กรุณาอนุญาตกล้อง หรือเล่นด้วยการแตะและคำสั่งเสียง";
  }
}
...
function renderChoices(question) {
  ...
  question.choices.forEach((choice, index) => {
    ...
    button.addEventListener("click", () =>
      selectAnswer(button, Number(choice))
    );
...
async function nextQuestion() {
  ...
  messageText.textContent =
    "ชี้นิ้วไปที่คำตอบแล้วจีบนิ้ว หรือกดปุ่มตอบด้วยเสียง";

  renderChoices(activeQuestion);

  const spokenQuestion =
    `${activeQuestion.left} คูณ ${activeQuestion.right} เท่ากับเท่าไร`;
  speak(spokenQuestion, 0.68);
}
...
async function selectAnswer(button, selected) {
  ...
  if (isCorrect) {
    ...
    messageText.textContent =
      `ยังไม่ถูก คำตอบที่ถูกคือ ${activeQuestion.answer}`;

    speak(
      `ยังไม่ถูกนะ คำตอบที่ถูกคือ ${activeQuestion.answer}`,
      0.68
    );

    answerZone.querySelectorAll("button").forEach((item) => {
      if (Number(item.textContent) === Number(activeQuestion.answer)) {
...
function findOrbByValue(value) {
  return [...answerZone.querySelectorAll(".answer-orb")].find(
    (orb) => Number(orb.textContent) === Number(value)
  );
}
...
function parseThaiNumberWords(text) {
  ...
  const hundredMatch = remaining.match(
    /(หนึ่ง|สอง|สาม|สี่|ห้า|หก|เจ็ด|แปด|เก้า)ร้อย/
  );
  ...
  const tenMatch = remaining.match(
    /(ยี่สิบ|สิบ|หนึ่งสิบ|สองสิบ|สามสิบ|สี่สิบ|ห้าสิบ|หกสิบ|เจ็ดสิบ|แปดสิบ|เก้าสิบ)/
  );

  if (tenMatch) {
    const token = tenMatch[1];
    if (token === "สิบ" || token === "หนึ่งสิบ") {
      total += 10;
    } else if (token === "ยี่สิบ" || token === "สองสิบ") {
      total += 20;
    } else {
      const prefix = token.replace("สิบ", "");
      total += (units[prefix] || 0) * 10;
    }
  ...
  const unitTokens = Object.keys(units).sort(
...
function setupVoiceRecognition() {
  ...
  recognition.onerror = (event) => {
    voiceStatus.textContent = `🎤 เสียง: ${event.error}`;
    messageText.textContent =
      "ไม่ได้ยินคำตอบชัดเจน กรุณากดไมค์แล้วพูดใหม่";
  };

  recognition.onresult = (event) => {
    const transcripts = [...event.results[0]].map(
      (result) => result.transcript
    );

    const parsedAnswers = transcripts
      .map(parseThaiNumberWords)
    ...
    if (!Number.isFinite(selected)) {
      messageText.textContent =
        `ได้ยินว่า “${transcripts[0]}” แต่ยังอ่านเป็นตัวเลขไม่ได้`;
      speak("กรุณาพูดคำตอบเป็นตัวเลขอีกครั้ง", 0.68);
      return;
    }

    messageText.textContent = `ได้ยินคำตอบ ${selected}`;
    const matchingOrb = findOrbByValue(selected);

    // สามารถตอบเลขที่ไม่อยู่ในตัวเลือกได้ ระบบยังตรวจคำตอบให้
    selectAnswer(matchingOrb, selected);
  };
}
...
function startVoiceAnswer() {
  ...
  if (!recognition) {
    messageText.textContent = "เบราว์เซอร์นี้ไม่รองรับคำสั่งเสียง";
    return;
  }
